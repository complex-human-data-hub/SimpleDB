...
class Domain(QuerySelect):
    ...
    def backup(self, bucket, prefix='backups', batch_size=250):
        """
        Backup all items in the domain to S3 as JSON files.

        Params:
            bucket: S3 bucket to store backup files
            prefix: S3 key prefix (default: 'backups')
            batch_size: number of items per backup file (default: 250)

        Returns dict with: total_items, file_count, s3_prefix
        """
        if not self.domain:
            raise Exception("Domain name is not set")

        s3_prefix = '{}/{}/{}/'.format(prefix, self.domain, date.today().isoformat())
        s3_client = self._get_S3_client()

        sql = "SELECT * FROM `{}`".format(self.domain)
        next_token = None
        batch = []
        file_count = 0
        total_items = 0
        ...
        while True:
            ...
            for item in response.get('Items', []):
                ...
                if len(batch) >= batch_size:
                    file_count += 1
                    key = '{}{:09d}.json'.format(s3_prefix, file_count)
        ...
        return {
            ...
            's3_prefix': 's3://{}/{}'.format(bucket, s3_prefix)
        }

    def restore(self, bucket, backup_date, source_domain, target_domain=None, prefix='backups', force=False):
        """
        Restore a domain backup from S3.

        Params:
            bucket: S3 bucket where backup files are stored
            backup_date: date string (YYYY-MM-DD) of the backup to restore
            source_domain: name of the domain whose backup to read
            target_domain: domain to restore into (defaults to self.domain)
            prefix: S3 key prefix (default: 'backups')
            force: if True, skip confirmation prompt when target has existing items

        Returns dict with: total_items, domain
        """
        if not target_domain:
            target_domain = self.domain
        if not target_domain:
            raise Exception("No target domain specified")

        s3_prefix = '{}/{}/{}/'.format(prefix, source_domain, backup_date)
        s3_client = self._get_S3_client()

        # List backup files
        response = s3_client.list_objects_v2(Bucket=bucket, Prefix=s3_prefix)
        backup_files = [obj['Key'] for obj in response.get('Contents', [])]
        while response.get('IsTruncated'):
            ...
            backup_files.extend([obj['Key'] for obj in response.get('Contents', [])])

        if not backup_files:
            raise Exception("No backup files found at s3://{}/{}".format(bucket, s3_prefix))

        # Check if target domain exists
        existing_domains = self.list_domains() or []
        if target_domain in existing_domains:
            # Check if it has items
            check = Domain(domain=target_domain, region=self.region, s3_bucket=getattr(self, 's3_bucket', ''))
            item_count = check.count()
            if item_count > 0:
                if not force:
                    answer = input(
                        "Domain '{}' already has {} items. Overwrite? (yes/no): ".format(
                            target_domain, item_count
                        )
                    )
            ...
            self.conn.create_domain(DomainName=target_domain)

        # Restore items from each backup file
        total_items = 0
        for file_key in sorted(backup_files):
            obj = s3_client.get_object(Key=file_key, Bucket=bucket)
            items = json.loads(obj['Body'].read())

            # batch_put_attributes accepts max 25 items per call
            for i in range(0, len(items), 25):
                chunk = items[i:i + 25]
                batch_items = []
                for item in chunk:
                    attrs = [
                        {'Name': a['Name'], 'Value': a['Value'], 'Replace': True}
